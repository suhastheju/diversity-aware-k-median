...
def local_search_v1(A, C, R, seed):
    ...
    iters = 0
    while(1):
        cur_cost, cur_S0, cur_S1, cur_iters = local_search_v1_iter(A, F0, F1, S0, S1, cost)
        ...
        for seed in seed_list:
            for min_frac in min_frac_list:
                ...
                for r0 in range(r0_min, k+1):
                    ...
                    tstart = time.time()
                    cur_cost, cur_S0, cur_S1, cur_iters =  local_search_v1(A, C, R, seed)
...
def main():
    output = open("results/report-local-search-v1-exp1.out", 'w')
    run_exp1(output)
    output.close()
